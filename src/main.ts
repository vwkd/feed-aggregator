...
export class FeedAggregator implements Disposable {
   ...
   */
  async #write(
    itemsPending: Map<string, AggregatorItem>,
    now: Date,
  ): Promise<void> {
    ...
    const items = itemsPending
      .entries()
      .map(([itemId, item]) => ({
        key: [...this.#prefix, ...(item.subprefix ?? []), itemId],
        value: item,
        type: "set" as const,
    ...
    for (const [itemId, item] of itemsPending.entries()) {
      this.#itemsStored.set(itemId, item);
    }

    logWrite.debug(
      `Wrote ${itemsPending.size} item${
        itemsPending.size > 1 ? "s" : ""
      } to database`,
    );
  }
  ...
  async add(...items: AggregatorItem[]): Promise<void> {
    ...
    const itemsPending: Map<string, AggregatorItem> = new Map();
